...
const foodSchema = new mongoose.Schema({
  ...
  quantity: {
    ...
    unit: {
      type: String,
      enum: ['grams', 'kilograms', 'milliliters', 'liters', 'pieces', 'packets', 'cans', 'bottles'],
      default: 'pieces'
    }
  },
  ...
  category: {
    type: String,
    enum: ['dairy', 'meat', 'fruits', 'vegetables', 'grains', 'snacks', 'beverages', 'condiments', 'frozen', 'canned', 'bakery', 'other'],
    default: 'other'
  },
  tags: [String],
  ...
  healthRisks: [{
    type: {
      type: String,
      enum: ['allergen', 'disease-risk', 'symptom-trigger', 'drug-interaction'],
      required: true
    },
    severity: {
      type: String,
...
foodSchema.methods.getHealthRiskSummary = function() {
  ...
  const harmfulRisks = this.healthRisks.filter(risk => risk.severity === 'harmful');
  const riskyRisks = this.healthRisks.filter(risk => risk.severity === 'risky');
