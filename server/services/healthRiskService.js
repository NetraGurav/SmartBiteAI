  ...
  async analyzeHealthRisk(food, userProfile) {
    ...
    try {
      ...
      if (userProfile.allergies && userProfile.allergies.length > 0) {
        risks.allergens = this.checkAllergenRisks(food, userProfile.allergies);
      }

      // Check for disease-related risks
      if (userProfile.diseases && userProfile.diseases.length > 0) {
        risks.diseases = this.checkDiseaseRisks(food, userProfile.diseases);
      }

      // Check for drug interactions
      if (userProfile.medications && userProfile.medications.length > 0) {
        risks.drugInteractions = this.checkDrugInteractions(food, userProfile.medications);
      }

      // Check for symptom triggers
      if (userProfile.symptoms && userProfile.symptoms.length > 0) {
        risks.symptoms = this.checkSymptomTriggers(food, userProfile.symptoms);
      }
    ...
    userDiseases.forEach(disease => {
      ...
      diseaseData.avoid.forEach(avoidItem => {
        if (ingredients.some(ingredient => ingredient.toLowerCase().includes(avoidItem.toLowerCase())) ||
            this.checkNutritionContent(nutrition, avoidItem)) {
          risks.push({
            type: 'disease',
            disease: disease,
            severity: 'risky',
